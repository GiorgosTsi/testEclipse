...
class BST implements BinarySearchTree { // Binary Search Tree implementation
	...
	private BinNode loadBinNode() throws IOException, ClassNotFoundException {
		long lchild_pos;
		long rchild_pos;
		...
		if (pos == 0)
			...
			Elem val = (Elem) ois.readObject();
			BinNode rt = new BinNodePtr(val);
			System.out.println("Reading: " + (Elem) rt.element());
			lchild_pos = ois.readLong();
			rchild_pos = ois.readLong();
			if (rchild_pos == -1)
				rt.setRight(null);
			else
				rt.setRight(loadBinNode());

			if (lchild_pos == -1)
				rt.setLeft(null);
			else
				rt.setLeft(loadBinNode());
